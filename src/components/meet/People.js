...
const People = ({people, containerDimensions}) => {
    ...
    const otherCount = people?.length > maxVisibleUsers ? people?.length - maxVisibleUsers : 0;
        ...
        <View style={peopleStyles.container}>
            {visiblePeople?.map((person, index) => {
                return (
                    <View
                        key={index}
                        style={[peopleStyles.card, Array.isArray(gridStyle) ? gridStyle[index] || gridStyle[0] : gridStyle]}
                    >
                        ...
                        {otherCount > 0 && index == visiblePeople?.length -1 && (
                            <TouchableOpacity
                            style={[peopleStyles.others]}
                            activeOpacity={0.8}>
                                <Text style={peopleStyles.othersText}> others </Text>  
                            </TouchableOpacity>
                        )}
